...
namespace jolt {
    namespace graphics {
        namespace vulkan {
            ...
            void Fence::reset_multiple(Fence const *const fences, uint32_t const n) {
            ...
            bool Fence::wait(uint64_t const timeout) const {
                ...
                switch(result) {
                    ...
                    default:
                        console.err("Error waiting for fence");
                        abort();
                }
            }

            bool Fence::wait_multiple(
              Fence *fences, uint32_t n, uint64_t const timeout, bool const all) {
                collections::Array<VkFence> raw_fences{n};
                ...
                switch(result) {
                    ...
                    default:
                        console.err("Error waiting for multiple fences");
                        abort();
                }
            }

            void Fence::create(bool const signaled) {
                VkFenceCreateInfo cinfo{
                  VK_STRUCTURE_TYPE_FENCE_CREATE_INFO, // sType
                  nullptr,                             // pNext
                  signaled ? VK_FENCE_CREATE_SIGNALED_BIT
                           : static_cast<VkFenceCreateFlags>(0) // flags
                };
            ...
            uint64_t Semaphore::wait(uint64_t timeout) const {
            ...
            void Semaphore::wait_multiple(
              Semaphore const *const semaphores,
              uint32_t const n,
            ...
            void Semaphore::create(VkSemaphoreType const sem_type, uint64_t const value) {
            ...
            void Semaphore::dispose() {
                if(m_semaphore) {
                    vkDestroySemaphore(
                      m_renderer.get_device(), m_semaphore, get_vulkan_allocator());
            ...
            void Semaphore::signal(uint64_t value) {
                jltassert2(
                  m_semaphore != VK_NULL_HANDLE, "Attempting to signal an invalid semaphore");
                jltassert2(
